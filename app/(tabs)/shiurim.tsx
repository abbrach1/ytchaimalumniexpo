...
export default function ShiurimScreen() {
  const { currentShiur, isPlaying, isLoading: audioLoading, playShiur, pauseResume } = useAudio();
  ...
  const loadShiurim = async () => {
    try {
      ...
      console.error('Shiurim load error:', e);
    } finally {
      setIsLoading(false);
      setRefreshing(false);
    }
        ...
        <TouchableOpacity
          ...
          <View style={styles.shiurLeft}>
            ...
            <View style={styles.shiurMeta}>
              <Text style={[styles.shiurTitle, isActive && styles.shiurTitleActive]} numberOfLines={2}>
                {item.title}
              </Text>
              <Text style={styles.shiurRebbeDate}>
                {item.rebbe} · {formatDate(item.date)}
              </Text>
              {item.series && (
                <Text style={styles.seriesText}>Series: {item.series}</Text>
              )}
            </View>
          </View>
          <Ionicons
        ...
        {isExpanded && (
          <View style={styles.shiurDetail}>
            ...
            <View style={styles.statsRow}>
              {item.playCount != null && (
                <Text style={styles.stat}>▶ {item.playCount} plays</Text>
              )}
            </View>
          </View>
        )}
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
        ...
        <View style={styles.searchBox}>
          ...
          {search ? (
            <TouchableOpacity onPress={() => setSearch('')}>
              <Ionicons name="close-circle" size={18} color={Colors.navyOpacity50} />
            </TouchableOpacity>
          ) : null}
        </View>
        <TouchableOpacity
      ...
      {hasFilters && (
        <View style={styles.activeFilters}>
          {selectedRebbeFilter && (
            <TouchableOpacity style={styles.filterChip} onPress={() => setSelectedRebbeFilter(null)}>
              <Text style={styles.filterChipText}>{selectedRebbeFilter} ✕</Text>
            </TouchableOpacity>
          )}
          {selectedTagFilter && (
            <TouchableOpacity style={styles.filterChip} onPress={() => setSelectedTagFilter(null)}>
              <Text style={styles.filterChipText}>{selectedTagFilter} ✕</Text>
            </TouchableOpacity>
          )}
        </View>
      )}
      ...
      {!isLoading && (
        <Text style={styles.countText}>{filtered.length} shiur{filtered.length !== 1 ? 'im' : ''}</Text>
      )}
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.empty}>
              <Ionicons name="musical-notes" size={40} color={Colors.navyOpacity30} />
              <Text style={styles.emptyText}>No shiurim found</Text>
            </View>
          }
        />
      )}
        ...
        <SafeAreaView style={styles.modalSafe}>
          ...
          <ScrollView style={styles.modalBody}>
            ...
            {allRebbeim.length > 0 && (
              <>
                ...
                <TouchableOpacity
                  style={styles.filterOption}
                  onPress={() => setSelectedRebbeFilter(null)}>
                ...
                {allRebbeim.map((r) => (
                  <TouchableOpacity
                    key={r}
                    style={styles.filterOption}
                    onPress={() => setSelectedRebbeFilter(r === selectedRebbeFilter ? null : r)}>
                ...
                <View style={styles.tagsGrid}>
                  {allTags.map((tag) => (
                    <TouchableOpacity
                      ...
                      <Text style={[styles.tagChipText, selectedTagFilter === tag && styles.tagChipTextActive]}>
                        {tag}
                      </Text>
                    </TouchableOpacity>
                  ))}
          ...
          <View style={styles.modalFooter}>
            <TouchableOpacity
              ...
              onPress={() => { setSelectedRebbeFilter(null); setSelectedTagFilter(null); setSortOrder('dateDesc'); }}>
...
const styles = StyleSheet.create({
  ...
  filterChip: {
    backgroundColor: Colors.goldOpacity15,
    borderRadius: 20,
    paddingHorizontal: 12,
    paddingVertical: 4,
  },
  ...
  clearBtn: {
    ...
    borderRadius: 12,
    borderWidth: 1,
    borderColor: Colors.navyOpacity30,
    alignItems: 'center',
  },
